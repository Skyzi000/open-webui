...
def add_terminal_agents_md(messages: list[dict], agents_md: str) -> list[dict]:
    ...
    from open_webui.utils.context_compaction import CONTEXT_COMPACTION_PREFIX_MARKER_KEY

    for index, message in enumerate(messages):
        if message.get('role') == 'user':
            return [
                *messages[:index],
                {'role': 'user', 'content': agents_md, CONTEXT_COMPACTION_PREFIX_MARKER_KEY: True},
                *messages[index:],
            ]
    return messages
