        ...
        try:
            ...
            if ctx is None:
                ctx = await build_chat_response_context(
                    request, form_data, user, model, metadata, tasks, events, compaction_state
                )
            else:
                ctx.update(form_data=form_data, metadata=metadata, events=events, compaction_state=compaction_state)
